...
const char *months[12][MONTH_LANGS] = {{"january", "jan", "gennaio", "gen"},
    {"february", "feb", "febbraio", "feb"}, {"march", "mar", "marzo", "mar"},
    {"april", "apr", "aprile", "apr"}, {"may", "may", "maggio", "mag"},
    {"june", "jun", "giugno", "giu"}, {"july", "jul", "luglio", "lug"},
...
int find_date(char *dest, const char *filename) {
    ...
    re = pcre2_compile(
        pattern, PCRE2_ZERO_TERMINATED, 0, &err_number, &err_offset, NULL);
    if (re == NULL) return FAILURE;

    // Perform the match
    match_data = pcre2_match_data_create_from_pattern(re, NULL);
    rc = pcre2_match(
        re, subject, strlen((char *)subject), 0, 0, match_data, NULL);
    ...
    if (endptr == year_str) return FAILURE;
    ...
    if (endptr == month_str) return FAILURE;
    ...
    if (endptr == day_str) return FAILURE;
